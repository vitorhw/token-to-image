...
export async function POST(req: NextRequest) {
  try {
    const { regions, prompt, referenceMapDataUrl, poseKeypoints } = await req.json();
    if (!regions?.length) {
      return NextResponse.json({ error: "regions required" }, { status: 400 });
    }
    const mapDataUrl = await generateSpatialMap(regions, prompt || "", referenceMapDataUrl, poseKeypoints);
